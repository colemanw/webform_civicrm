...
-However, due to a limitation of the CiviCRM API, if you include any custom fields on your form, your users (including anonymous users if they will be using the form) MUST have "access all custom data" permission in order for those fields to be processed.
...
NOTE: If you are sending an HTML (rich-text) message, the recommended method is to highlight a piece of text in your message such as "click here for the form" then click the link button and then paste in the above-mentioned url.
